...
public class RSSFeedParserMarkets {
	...
	public RSSFeedParserMarkets(String filename) {
			...
			while (eventReader.hasNext()) {
				...
				if (event.isStartElement()) {
					...
					case DESCRIPTION:
						...
						String maindesc=getCharacterData(event, eventReader);
						//System.out.println("maindesc"+maindesc);
						if (isFeedHeader) {
							isFeedHeader = false;
							feed = new FeedMarkets(title, description, link, image, guid, pubdate, url, coverImages,
									src);
						}
						//System.out.println("Description data are"+description);
						event = eventReader.nextEvent();
						description = getCharacterData(event, eventReader);
						// description.getBytes("src=");
						
						String[] subdesc = description.split("<img");
						for(int i=0;i< subdesc.length;i++)
						{
						//System.out.println("desc1"+subdesc[i]);
...
//					}
					if (event.asEndElement().getName().getLocalPart() == (ITEM)) {
						...
						message.setTitle(title);
						message.setDescription(description.replaceAll("\"", ""));
						message.setLink(link);
