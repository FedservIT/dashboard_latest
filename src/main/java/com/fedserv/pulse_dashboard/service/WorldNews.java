...
public class WorldNews {
	public Object getOutput() throws Exception {
		String cat = "World";
		String url = "https://www.business-standard.com/rss/international-116.rss";
		...
		if (cat.contentEquals("World")) {
			for (FeedMessage message : feed.getMessages()) {
				if (cat.contentEquals("World")) {
					...
					System.out.println("---world--->"+strDate );
					jsonArr.put(obj);
				} 
			}
		}
		...
		try {
			collectionsApiFuture = firestoreDb.collection("newscenter_test")
					.document("World").set(firebaseModel);
			System.out.println("saving world data to firebase"+collectionsApiFuture);
			...
			//newsstatus.put("updatetime", collectionsApiFuture.get().getUpdateTime().toString());
			//return 	newsstatus;
		} catch (Exception e) {
